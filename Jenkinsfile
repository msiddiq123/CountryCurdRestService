 ...
 environment{
   ...
   post{
	...
	echo '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> Executing post handler - always >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>'
        ...
        body: "Please find the build and log details at ${env.BUILD_URL} .... Log file is available at ${env.JENKINS_HOME}/jobs/${env.JOB_NAME}/branches/${env.BRANCH_NAME}/builds/${env.BUILD_NUMBER}/log" 	
     }
